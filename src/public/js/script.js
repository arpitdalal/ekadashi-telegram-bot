...
const fetchEkadashis = () => {
  fetch('/ekadashis')
    .then((res) => res.json())
    .then((data) => {
      document.querySelector('#existingEkadashis').innerHTML = JSON.stringify(
        data,
        null,
        2,
      );
...
window.addEventListener('load', () => {
  ...
  document.querySelector('#submit').addEventListener('click', () => {
    ...
    const result = confirm(
      'This will override the Ekadashis file permanently. Do you want to go through?',
    );
    if (!result) {
      document.querySelector('#submit').disabled = false;
      document.querySelector('#submit').textContent = 'Submit';
    ...
    fetch('/ekadashis', {
      ...
      .then((data) => {
        if (data.ok === 'true') {
          document.querySelector('#successOrError').innerHTML =
            "<p class='green'>Ekadashis file has been successfully updated!</p>";
          setTimeout(fetchEkadashis, 500);
        } else {
          document.querySelector(
            '#successOrError',
          ).innerHTML = `<p class='red'>Couldn't update the ekadashis file!</p>`;
          console.log(data.message);
        }
      })
      .catch((err) => {
        document.querySelector('#successOrError').innerHTML =
          "<p class='red'>Couldn't update the ekadashis file!</p>";
  ...
  document.querySelector('#loadCurrent').addEventListener('click', () => {
    document.querySelector('#newEkadashis').innerHTML =
      document.querySelector('#existingEkadashis').innerHTML;
  });
  ...
  document.querySelector('#edit').addEventListener('click', () => {
    if (isEditable) {
      ...
      const isValidJSON = isJSON(
        document.querySelector('#newEkadashis').textContent,
      );
      if (!isValidJSON) {
        alert('Invalid JSON!');
        return;
